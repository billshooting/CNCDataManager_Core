...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class SpindleBeltLengthsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutSpindleBeltLength(int id, [FromBody] SpindleBeltLength spindleBeltLength)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostSpindleBeltLength([FromBody] SpindleBeltLength spindleBeltLength)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
