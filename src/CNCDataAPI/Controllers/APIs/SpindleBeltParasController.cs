...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class SpindleBeltParasController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutSpindleBeltPara(string id, [FromBody] SpindleBeltPara spindleBeltPara)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostSpindleBeltPara([FromBody] SpindleBeltPara spindleBeltPara)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
