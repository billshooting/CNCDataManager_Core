...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class HubShapedCoupsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutHubShapedCoup(string id, [FromBody] HubShapedCoup hubShapedCoup)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostHubShapedCoup([FromBody] HubShapedCoup hubShapedCoup)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
