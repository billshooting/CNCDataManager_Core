...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class AlignBallBrgsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutAlignBallBrg(string id, [FromBody]AlignBallBrg alignBallBrg)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostAlignBallBrg([FromBody]AlignBallBrg alignBallBrg)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
