...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class SpindleSrvMotorSizesController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutSpindleSrvMotorSize(string id, [FromBody] SpindleSrvMotorSize spindleSrvMotorSize)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostSpindleSrvMotorSize([FromBody]SpindleSrvMotorSize spindleSrvMotorSize)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
