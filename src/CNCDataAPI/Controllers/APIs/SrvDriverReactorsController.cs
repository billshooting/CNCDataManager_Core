...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class SrvDriverReactorsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutSrvDriverReactor(string id, [FromBody] SrvDriverReactor srvDriverReactor)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostSrvDriverReactor([FromBody] SrvDriverReactor srvDriverReactor)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
