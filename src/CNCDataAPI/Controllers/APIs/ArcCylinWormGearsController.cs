...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class ArcCylinWormGearsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutArcCylinWormGear(string id, [FromBody]ArcCylinWormGear arcCylinWormGear)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostArcCylinWormGear([FromBody]ArcCylinWormGear arcCylinWormGear)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
