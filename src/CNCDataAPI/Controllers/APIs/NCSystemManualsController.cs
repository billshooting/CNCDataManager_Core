...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class NCSystemManualsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutNCSystemManual(string id, [FromBody] NCSystemManual nCSystemManual)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostNCSystemManual([FromBody] NCSystemManual nCSystemManual)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
