...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class StepMotorSizesController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutStepMotorSize(string id, [FromBody] StepMotorSize stepMotorSize)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostStepMotorSize([FromBody] StepMotorSize stepMotorSize)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
