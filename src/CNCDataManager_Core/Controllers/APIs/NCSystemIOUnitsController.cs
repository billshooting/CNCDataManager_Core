...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class NCSystemIOUnitsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutNCSystemIOUnit(string id, NCSystemIOUnit nCSystemIOUnit)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostNCSystemIOUnit(NCSystemIOUnit nCSystemIOUnit)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
