...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class CommonCylinWormGearsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutCommonCylinWormGear(string id, CommonCylinWormGear commonCylinWormGear)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostCommonCylinWormGear(CommonCylinWormGear commonCylinWormGear)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
