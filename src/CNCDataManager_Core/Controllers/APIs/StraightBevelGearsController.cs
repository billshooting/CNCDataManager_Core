...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class StraightBevelGearsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutStraightBevelGear(string id, StraightBevelGear straightBevelGear)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostStraightBevelGear(StraightBevelGear straightBevelGear)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
