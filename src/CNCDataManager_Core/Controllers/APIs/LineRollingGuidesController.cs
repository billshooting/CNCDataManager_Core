...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class LineRollingGuidesController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutLineRollingGuide(string id, LineRollingGuide lineRollingGuide)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostLineRollingGuide(LineRollingGuide lineRollingGuide)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
