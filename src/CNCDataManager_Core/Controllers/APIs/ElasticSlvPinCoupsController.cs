...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class ElasticSlvPinCoupsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutElasticSlvPinCoup(string id, ElasticSlvPinCoup elasticSlvPinCoup)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostElasticSlvPinCoup(ElasticSlvPinCoup elasticSlvPinCoup)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
