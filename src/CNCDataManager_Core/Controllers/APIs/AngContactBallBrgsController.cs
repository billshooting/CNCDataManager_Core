...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class AngContactBallBrgsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutAngContactBallBrg(string id, AngContactBallBrg angContactBallBrg)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostAngContactBallBrg(AngContactBallBrg angContactBallBrg)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
