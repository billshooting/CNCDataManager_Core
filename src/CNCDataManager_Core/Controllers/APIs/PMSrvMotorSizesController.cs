...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class PMSrvMotorSizesController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutPMSrvMotorSize(string id, PMSrvMotorSize pMSrvMotorSize)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostPMSrvMotorSize(PMSrvMotorSize pMSrvMotorSize)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
