...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class PMSrvMotorParasController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutPMSrvMotorPara(string id, PMSrvMotorPara pMSrvMotorPara)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostPMSrvMotorPara(PMSrvMotorPara pMSrvMotorPara)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
