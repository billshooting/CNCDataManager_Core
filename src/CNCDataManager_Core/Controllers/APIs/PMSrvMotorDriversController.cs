...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class PMSrvMotorDriversController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutPMSrvMotorDriver(string id, PMSrvMotorDriver pMSrvMotorDriver)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostPMSrvMotorDriver(PMSrvMotorDriver pMSrvMotorDriver)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
