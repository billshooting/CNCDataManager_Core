...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class ElecSpindleSizesController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutElecSpindleSize(string id, ElecSpindleSize elecSpindleSize)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostElecSpindleSize(ElecSpindleSize elecSpindleSize)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
