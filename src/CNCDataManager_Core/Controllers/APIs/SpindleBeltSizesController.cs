...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class SpindleBeltSizesController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutSpindleBeltSize(string id, SpindleBeltSize spindleBeltSize)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostSpindleBeltSize(SpindleBeltSize spindleBeltSize)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
