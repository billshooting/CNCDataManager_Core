...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class NCSystemPowerUnitsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutNCSystemPowerUnit(string id, NCSystemPowerUnit nCSystemPowerUnit)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostNCSystemPowerUnit(NCSystemPowerUnit nCSystemPowerUnit)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
