...
namespace CNCDataManager.Controllers.APIs
{
    ...
    public class HeliCylinGearsController : Controller
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutHeliCylinGear(string id, HeliCylinGear heliCylinGear)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> PostHeliCylinGear(HeliCylinGear heliCylinGear)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
